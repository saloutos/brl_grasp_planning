...
def single_object_experiment_panda(obj_name, obj_nominal_pose, planner_name, num_plans):
    ...
    while PandaGP.mj_viewer.is_running() and PandaGP.num_cycles <= total_cycles:
        if not PandaGP.paused:
            ...
            if PandaGP.ready_to_plan:
                ...
                if PandaGP.plan_perturbation == 0:
                    ...
                    try:
                        ...
                        PandaGP.experiment_data[PandaGP.num_cycles].update({'plan_success': True})
                    except:
                        print("Grasp planning failed.")
                        grasp_poses_world = np.array([create_SE3(trans=[0, -0.2, 0.2], rpy=[0, 0, -90])])
                        grasp_scores = np.array([0.0])
                        grasp_widths = np.array([0.08])
                        PandaGP.experiment_data[PandaGP.num_cycles].update({'plan_success': False})
            ...
            if PandaGP.ready_to_reset:
                ...
                # if done with perturbations, add some noise to object pose
                if PandaGP.plan_perturbation == 0:
                    new_pos = copy.deepcopy(obj_pos)
                    new_pos[0] += np.random.uniform(-0.1, 0.1)
                    new_pos[1] += np.random.uniform(-0.1, 0.1)
                    new_rpy = copy.deepcopy(obj_rpy)
                    new_rpy[2] += np.random.uniform(-90, 90)
                    new_quat = Rotation.from_euler('ZYX', new_rpy, degrees=True).as_quat()
                    PandaGP.experiment_data[PandaGP.num_cycles].update({
                        'new_obj_pos': new_pos,
                        'new_obj_rpy': new_rpy,
                    })
                    start_idx = PandaGP.mj_model.joint(obj_name).qposadr[0]
                    print(start_idx)
                    PandaGP.mj_model.keyframe(0).qpos[start_idx:start_idx+3] = new_pos
                    PandaGP.mj_model.keyframe(0).qpos[start_idx+3:start_idx+7] = new_quat
    ...
    PandaGP.experiment_data['success'] = successes

    exp_toc = time.time() - exp_tic

    PandaGP.experiment_data['exp_time'] = exp_toc

    print("Experiment completed in {:.2f} seconds.".format(exp_toc))

    output = copy.deepcopy(PandaGP.experiment_data)

    # shutdown
    PandaGP.shutdown()

    # clean up
    del spec, mj_model, planner, controller, PandaGP
